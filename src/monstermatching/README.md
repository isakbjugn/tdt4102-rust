...
Rust har et kraftig system for mønstermatching med [uttømmende sjekk](../ordliste.md#uttommende-sjekk), vakter og destrukturering på alle nivåer. C++ tilbyr `switch` for heltall og `std::variant` med `std::visit` (C++17) for type-sikre unioner — men med mer begrenset syntaks og ingen kompilatorgaranti for uttømmenhet.
